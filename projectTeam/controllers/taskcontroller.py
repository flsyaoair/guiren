...
def detail(task_id):
    ...
    if task.AssignTo == g.user_id:
        task.AssignTo = -1
    return render_template('Task/Detail.html',Task=task,HistoryList=history_list,Creator=task.CreatorProfile.Nick,ProjectKey=task.ProjectProfile.ProjectKey,MemberList=member_list,CurrentUser=g.user_id,ProjectModules=projectmodules,ModuleRelation=modulerelation,commitversion=cmversion)
